...
function implementFn<T extends ReturnType<typeof $.method>>(args: {
  ...
  if (hasFlatFields || auth.length) {
    ...
    // `security` and `auth` are passed positionally, so an operation parameter
    // of either name cannot repeat the keyword.
    const buildParamsArgs = [
      fieldsList,
      ...(auth.length ? [securityListNode(auth), $('self').attr('auth')] : []),
